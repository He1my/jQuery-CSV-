 ...
 * check it out.
 *
 * A special thanks goes out to Evan Plaice and to rwk@acm.org for providing a lot of valuable
  ...
  $.csv = {
    ...
    fromObjects2CSV: function(objects, options, callback) {
      ...
      config.experimental = 'experimental' in options ? options.experimental : false;
        ...
        var out = [];
        //headers are taken from the first object only
        var header=[];
        for (var h in objects[0]){
            header.push(h);
        }
        out.push(header.join(config.separator));
        ...
        for (t in objects){
            ...
            for(var propertyName in raw ) {
                ...
                line.push( raw[propertyName]);
            }
            out.push(line.join(config.separator));
        }
